...
def main(methods=None, force_rerun_original=False):
    ...
    orig_cache = os.path.join(
        config.OUTPUT_ROOT,
        f"_original_wy{config.WEATHER_YEAR}_results.pkl")
    price_cache = os.path.join(
        config.OUTPUT_ROOT,
        f"_co2_price_wy{config.WEATHER_YEAR}.pkl")
    ...
    if results_orig is None:
        ...
        if n_orig_solved is not None:
            n_orig_solved.export_to_netcdf(os.path.join(
                config.OUTPUT_ROOT,
                f"_dispatched_original_wy{config.WEATHER_YEAR}.nc"))

    figs_orig = rep.make_figures(results_orig, methods[0], f'Original wy{config.WEATHER_YEAR}', suffix='_original')

    all_results = {}
    for method in methods:
        log.info("=" * 70); log.info(f"METHOD: {method}")
        # build modified network
        mod_file, cf_change_df, demand_summaries = bmn.build(method)
        # v16 climate-signal tables for heating and cooling
        try:
            tdir = rep._dirs(method)['tables']
            ...
            log(f"  (climate-signal demand tables skipped: {e})")
        # dispatch modified
        n_mod = pypsa.Network(mod_file)
        n_mod = dsp.prepare_for_dispatch(n_mod, f'MOD-{method}', log.info, co2_price=co2_price)
        n_mod_solved, dt_m, mode_m = dsp.dispatch(n_mod, f'MOD-{method}', log.info)
        if n_mod_solved is None:
            log.error(f"  {method}: modified dispatch failed"); continue
        results_mod = dsp.extract_results(n_mod_solved, dt_m)
        n_mod_solved.export_to_netcdf(
            os.path.join(config.run_dir(method), 'networks',
                         f"dispatched_{method}_wy{config.WEATHER_YEAR}_c2e{config.C2E_FUTURE}.nc"))
        # report
        figs_mod = rep.make_figures(results_mod, method, f'{method.upper()} (C2E {config.C2E_FUTURE})',
                                    cf_change_df=cf_change_df)
        figs_orig_m = rep.make_figures(results_orig, method, f'Original wy{config.WEATHER_YEAR}', suffix='_original')
        rpt = rep.build_method_report(results_orig, results_mod, method, figs_orig_m, figs_mod)
        log.info(f"  report: {rpt}")
        all_results[method] = {'orig': results_orig, 'mod': results_mod}
        # cache this method's results so the comparison can include methods run
        # in SEPARATE invocations
        try:
            with open(os.path.join(config.OUTPUT_ROOT,
                                   f"_method_{method}_wy{config.WEATHER_YEAR}_c2e{config.C2E_FUTURE}.pkl"), 'wb') as f:
                pickle.dump(results_mod, f)
        except Exception as e:
            log.info(f"  (could not cache {method} results: {e})")

    # build comparison from ALL method caches present (so running methods one at
    # a time still produces a complete cross-method comparison)
    combined = {}
    for m in ['qdm', 'direct']:
        mc = os.path.join(config.OUTPUT_ROOT,
                          f"_method_{m}_wy{config.WEATHER_YEAR}_c2e{config.C2E_FUTURE}.pkl")
    ...
    if combined:
        ...
        log.info(f"COMPARISON ({len(combined)} method(s)): {cmp}")
