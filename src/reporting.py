...
def _pct(a, b):
    return (b - a) / a * 100 if abs(a) > 1e-9 else float('nan')


METHOD_BLURB = {
    'qdm': ("Quantile Delta Mapping (Cannon et al. 2015) - PRIMARY METHOD. The network keeps "
            "its own weather-year chronology; wind, solar, ror and hydro inflow are reshaped "
            "quantile by quantile by the C2E future/baseline change, heating demand by DAILY "
            "quantile-delta multipliers (3-month centred windows), and cooling by the "
...
def build_method_report(results_orig, results_mod, method, figs_orig, figs_mod):
    ...
    doc.add_heading('10. Summary tables', 1)
...
def build_comparison_report(all_results):
    cmp_dir = os.path.join(config.SESSION_DIR, f"COMPARISON_wy{config.WEATHER_YEAR}_c2e{config.C2E_FUTURE}")
    FIG = os.path.join(cmp_dir, 'figures'); TAB = os.path.join(cmp_dir, 'tables')
    os.makedirs(FIG, exist_ok=True); os.makedirs(TAB, exist_ok=True)

    doc = Document()
    doc.add_heading('Method comparison: two ways of imposing the future climate', 0)
    doc.add_paragraph(
        "The same fixed network is stress-tested under two coherent constructions of the C2E "
        "future. QDM keeps the network's own weather-year chronology and reshapes each variable "
        "quantile by quantile, isolating the climate-change signal. DIRECT adopts the C2E "
        "future year wholesale, which also carries conversion-chain differences between C2E and "
        "the network's atlite world, and is Gotske's own weather-year stress philosophy "
        "extended to a future year. The spread between them brackets the result: QDM is the "
        "clean climate signal, DIRECT is the fuller-stress modelled world.")
    ...
    doc.add_paragraph(
        "Where DELTA diverges sharply from QDM, the divergence is attributable to changes in "
        "variability that DELTA cannot represent. Lead with QDM; report all three.")
    out = os.path.join(cmp_dir, f'COMPARISON_wy{config.WEATHER_YEAR}_c2e{config.C2E_FUTURE}.docx')
    doc.save(out)
    return out
